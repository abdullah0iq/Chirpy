...
const getAllChirpsByAuthorIdAsc = `-- name: GetAllChirpsByAuthorIdAsc :many
SELECT id, created_at, updated_at, body, user_id FROM chirps
WHERE user_id = $1
ORDER BY created_at ASC
`

func (q *Queries) GetAllChirpsByAuthorIdAsc(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getAllChirpsByAuthorIdAsc, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const getAllChirpsByAuthorIdDesc = `-- name: GetAllChirpsByAuthorIdDesc :many
SELECT id, created_at, updated_at, body, user_id FROM chirps
WHERE user_id = $1
ORDER BY created_at DESC
`

func (q *Queries) GetAllChirpsByAuthorIdDesc(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getAllChirpsByAuthorIdDesc, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
